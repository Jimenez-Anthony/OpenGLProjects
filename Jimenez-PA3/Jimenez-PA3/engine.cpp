...
void Engine::ProcessInput()
{
    ...
    if (glfwGetKey(m_window->getWindow(), GLFW_KEY_UP) == GLFW_RELEASE &&
        ...
        m_graphics->getPyramid()->setSpeed(glm::vec3(0., 0., 0.));
        m_graphics->getMoon()->setSpeed(glm::vec3(0., 0., 0.));

    }
    if (glfwGetKey(m_window->getWindow(), GLFW_KEY_UP) == GLFW_PRESS) {
        m_graphics->getPyramid()->setSpeed(glm::vec3(0., .1, 0.));
        m_graphics->getCube()->setSpeed(glm::vec3(0., .1, 0.));
        m_graphics->getMoon()->setSpeed(glm::vec3(0., .1, 0.));
    ...
    if (glfwGetKey(m_window->getWindow(), GLFW_KEY_LEFT) == GLFW_PRESS) {
        m_graphics->getPyramid()->setSpeed(glm::vec3(.1, 0., 0.));
        m_graphics->getCube()->setSpeed(glm::vec3(-.1, 0., 0.));
        m_graphics->getMoon()->setSpeed(glm::vec3(-.1, 0., 0.));
...
void Engine::Display(GLFWwindow* window, double time) {
    ...
    m_graphics->Render(time, speed, window);
