...
void Graphics::Update(double dt)
{
	...
	speed = { 0.1,0.0,0.1 };
	dist = { 6., 0., 6. };
	rotSpeed = { 0.5f, 0.5f, 0.0f };
	scale = { .75f, .75f, .75f };
	rotVector = glm::vec3(.5, .0, .5);
	ComputeTransforms(dt, speed, dist, rotSpeed, rotVector, scale, tmat, rmat, smat);
	if(m_mesh!=NULL)
...
void Graphics::Render()
{
	...
	if (m_sphere2 != NULL){
		...
		m_sphere2->Render(m_positionAttrib,m_colorAttrib);
	}
	...
	if (m_mesh != NULL) {
		glUniformMatrix4fv(m_modelMatrix, 1, GL_FALSE, glm::value_ptr(m_mesh->GetMesh()));
