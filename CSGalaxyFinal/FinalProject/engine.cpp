...
{
    Mesh* starShip = m_graphics->getMesh();
    ...
    float cameraSpeed = static_cast<float>(.02 * glfwGetTime());
    ...
    m_graphics->getCamera()->Update(cameraPos, cameraFront, cameraUp, fov);
    m_graphics->getMesh()->Update(glm::vec3(cameraPos.x, cameraPos.y - 1, cameraPos.z - 3), cameraFront, cameraUp);
