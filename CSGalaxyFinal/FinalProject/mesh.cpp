...
Mesh::Mesh(glm::vec3 pivot, float scale, const char* fname)
{
	// Vertex Set Up
...
Mesh::Mesh(glm::vec3 pivot, float scale, const char* fname, const char* tname)
{
	// Vertex Set Up
	loadModelFromFile(fname);
...
void Mesh::setupModelMatrix(glm::vec3 pivot, float angle, float scale) {
	...
	model *= glm::scale(glm::vec3(.05, .05, .05));
}
...
void Mesh::Update(glm::vec3 cameraPos, glm::vec3 cameraFront, glm::vec3 cameraUp)
{
	model = glm::translate(glm::mat4(1.0f), cameraFront + cameraPos);	
	model *= glm::scale(glm::vec3(.05, .05, .05));
	model *= glm::inverse(glm::lookAt(glm::vec3(0, 0, 0), -cameraFront, glm::vec3(0, 1, 0)));

	//setupModelMatrix(cameraPos, glm::radians(180.0f), .05);
